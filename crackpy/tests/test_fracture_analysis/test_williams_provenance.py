...
def test_williams_provenance_artifact_writer_accepts_explicit_result_envelope(tmp_path):
    ...
    assert written["kg_statement_bundle"].name == "explicit_result_williams_fit_kg_statement_bundle.json"
    assert written["visualization_graph"].name == "explicit_result_williams_fit_graph.json"
    assert written["visualization_graph_html"].name == "explicit_result_williams_fit_graph.html"
    assert written["envelope"].exists()
    assert written["kg_statement_bundle"].exists()
    assert written["visualization_graph"].exists()
    assert written["visualization_graph_html"].exists()
