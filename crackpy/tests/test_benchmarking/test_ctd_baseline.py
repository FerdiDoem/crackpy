...
def test_coordinate_head_is_denormalized_as_a_separate_diagnostic() -> None:
    normalized = torch.tensor([-0.5, 0.5])

    assert denormalize_coordinate_head(normalized, model_pixels=256) == pytest.approx((64.0, 192.0))
    assert map_model_point_to_original((64.0, 192.0), model_pixels=256, original_pixels=128) == (32.0, 96.0)
    assert map_model_point_to_original(None, model_pixels=256, original_pixels=128) is None
...
def test_crackmnist_keeps_failures_missing_references_and_per_sample_records() -> None:
    ...
    assert result["samples"][0]["coordinate_head_diagnostic_rc"] == [64.0, 64.0]
    json.dumps(result, allow_nan=False)
    assert all(not parameter.requires_grad for parameter in _FakeParallelNets().parameters())
