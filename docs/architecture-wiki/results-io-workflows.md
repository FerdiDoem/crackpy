...
- Generic method-runtime update: backend integration can use `crackpy.methods.runtime.MethodRunIdentityPolicy` to derive deterministic configuration, run, and result IDs for Williams/CJP-style method envelopes, including optional variant IDs such as CJP Mode-I. `build_manual_crack_tip_estimate()` projects a `CrackTipFrame` into the same imported-crack-tip estimate shape already used by current provenance envelopes. These helpers are importable infrastructure only in this slice; existing Williams-fit and CJP-fit envelope payloads and filenames are unchanged until their builders explicitly delegate to them.
