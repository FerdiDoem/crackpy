...
- C-004 workflow-runner update: backend/frontend integration can use `crackpy.fracture_analysis.workflow.FractureWorkflowInput` and `run_fracture_analysis_workflow()` when it needs one fracture-analysis run without also invoking legacy text writing, JSON writing, plot generation, output-folder policy, or multiprocessing policy.
  The workflow input exposes `crack_tip_id` as the durable crack-tip key.
  `compatibility_side` is retained only as the current `left`/`right` adapter label for `CrackTipInfo`, filenames, and plot labels.
